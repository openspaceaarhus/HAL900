...
#define SUB4(x)  ((((uint32_t)s_box[((x) & 0xFF000000) >> 24]) << 24) | (((uint32_t)s_box[((x) & 0xFF0000) >> 16]) << 16) | (((uint32_t)s_box[((x) & 0xFF00) >> 8]) << 8) | s_box[((x) & 0xFF)])
...
void aes_init(uint32_t *round_key, counter b, const uint32_t *key, counter n)
{
  uint32_t t;
