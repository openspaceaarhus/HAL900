...
public class ByteBuffer {
    ...
    public ByteBuffer() {
        try {
            ...
            log.log(Level.SEVERE, "Impossible case reached constructing ByteBuffer");
        }
    }
    ...
    public ByteBuffer(byte[] bytes, int offset, int length, int capacity)
        ...
        try {
            ...
            log.log(Level.SEVERE, "Impossible case of IllegalStateException in ByteBuffer constructor", e);
        }
    }
    ...
    public ByteBuffer(String string0, int capacity) throws IllegalArgumentException {
        ...
        try {
            ...
            log.log(Level.SEVERE, "Impossible case in BtyeBuffer(String) constructor since ISO-8859-1 should be supported encoding", e);
        ...
        try {
            ...
            log.log(Level.SEVERE, "Impossible case in BtyeBuffer(String) constructor since capacity should have been ensured", e);
            throw new IllegalArgumentException("IllegalStateException exception thrown, should never happen", e);
        }
    }
    ...
    public void add(byte b){
        if (isFull()) {
            throw new IllegalStateException("Buffer is full and has reached maximum capacity (" + capacity() + ")");
        }
     ...
     * is not large enough, this method will throw a IllegalStateException. A byte
     ...
     * @throws IllegalStateException If this buffer's free space is not large enough to store add the byte array
     */
    public void add(byte[] bytes) throws IllegalStateException {
     ...
     * is not large enough, this method will throw a IllegalStateException.
    ...
    public void add(byte[] bytes, int offset, int length)
        ...
        if (length > free()) {
            throw new IllegalStateException("Buffer does not have enough free space (" + free() + " bytes) to add " + length + " bytes of data");
        }
        ...
        for (int i = 0; i < length; i++) {
            try {
                ...
                log.log(Level.SEVERE, "Buffer is full even though this method checked its size() ahead of time", e);
     ...
     * methods.
     */
    public void delete(int count) throws IllegalStateException {
        if ((count < 0) || (count > capacity())) {
            throw new IllegalArgumentException("Can only delete between 0 and " + capacity() + " bytes from buffer, you passed in=" + count);
        }
        if (count > size()) {
            throw new IllegalStateException("Buffer size (" + size() + ") not large enough to delete (" + count + ") bytes");
        }
    ...
    public byte remove() {
        if (size() == 0) {
            throw new IllegalStateException("Buffer is empty and no bytes available to remove");
        }
     ...
     * requested is larger than the buffer size, then a <code>IllegalStateException</code>
     * will be thrown.
     ...
     * @throws IllegalStateException If the buffer is not large enough to fufill the request
     *      (if N is > size)
     */
    public byte[] remove(int count) throws IllegalStateException {
        if ((count < 0) || (count > capacity())) {
        ...
        if (count > size()) {
            throw new IllegalStateException("Buffer size (" + size() + ") not large enough to remove (" + count + ") bytes");
        }
        ...
        for (int i = 0; i < count; i++, currentPos++) {
            try {
                ...
                // this should be an impossible case since we checked the size() above
                log.log(Level.SEVERE, "Buffer is empty even though this method checked its size() ahead of time", e);
                throw new IllegalStateException(e.getMessage());
            }
        }
    ...
    /**
     ...
     while (offset < size()) {
     log.debug("in split loop for i=" + i + ",delimiterPos=" + delimiterPos + ",offset=" + offset + ",tokens=" + tokens + ",size=" + this.size());
     // find the next delimiter occurrence
     delimiterPos = indexOf(delimiter, offset);
     log.debug("found next delimiter at pos=" + delimiterPos);
     ...
     * @throws IllegalStateException If the index is >= size()
     */
    public byte get(int index) throws IllegalArgumentException, IllegalStateException {
        if ((index < 0) || (index  >= capacity())) {
            throw new IllegalArgumentException("The buffer index must be a value between 0 and " + capacity() + "!");
        }
        if (index >= size()) {
            throw new IllegalStateException("Index " + index + " is >= buffer size of " + size());
        }
        return this.buffer[(this.currentReadPosition + index) % this.buffer.length];
    }
    ...
    public boolean endsWith(byte[] prefix) {
        ...
        while (match && (i >= 0)) {
            //log.debug("endsWith() this.buffer["+j+"]=" + this.buffer[j] + "='" + (char)this.buffer[j] + "', prefix["+i+"]=" + prefix[i] + "='" + (char)prefix[i] + "'");
            if (this.buffer[j] != prefix[i]) {
                match = false;
            }
            i--;
    ...
    public boolean equals(Object obj)  {
        ...
        for (int i = 0; i < length; i++) {
            try {
                ...
                log.log(Level.SEVERE, "Impossible case should never happen", e);
                return false;
            }
        }
